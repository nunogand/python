...
def revistas():
  for archive in range(15):
    print ('Ler indices:',url+str(archive))
    f = requests.get(url+str(archive))
    #f = requests.get(url)
...
df.to_csv('acta_medica.csv', sep='|', encoding='utf-8')
df.to_excel('acta_medica.xlsx')